...
export function createSketchServer(portOrOptions: number | SketchServerOptions): SketchServer {
      ...
      switch (msg.type) {
        case "reorder": {
          if (!isProjectFilePathSafe(msg.filePath, projectRoot)) {
            ...
            logger.warn(`[Glide] Rejected reorder path: ${msg.filePath}`);
        ...
        case "updateProperties": {
          if (!isProjectFilePathSafe(msg.filePath, projectRoot)) {
            const error = msg.filePath.trim()
              ? "File path is outside the project root"
              : "File path could not be resolved for this element";
            logger.warn(`[Glide] Rejected property update path: ${msg.filePath}`);
        ...
        case "updateText": {
          if (!isProjectFilePathSafe(msg.filePath, projectRoot)) {
            const error = msg.filePath.trim()
              ? "File path is outside the project root"
              : "File path could not be resolved for this element";
            logger.warn(`[Glide] Rejected text update path: ${msg.filePath}`);
        ...
        case "revertChanges": {
          logger.debug(`[revertChanges] Reverting ${msg.undoIds.length} undo IDs:`, msg.undoIds);
          ...
          logger.debug(`[revertChanges] Results:`, results);
  ...
  wss.on("connection", (ws) => {
    ...
    try {
      ...
      logger.warn("[Glide] Could not resolve Tailwind config:", err);
    }

    // Send existing undo history so the overlay can rebuild its changelog
    // (handles page reloads caused by HMR)
    const activeUndos = undoStack.filter(e => !e.reverted);
    if (activeUndos.length > 0) {
      const undoIds = activeUndos.map(e => e.id);
      logger.debug(`[connection] Sending ${undoIds.length} active undo entries to new client`);
      send(ws, { type: "undoHistory", undoIds });
    }
    ...
    ws.on("message", (data) => {
      ...
      switch (msg.type) {
        ...
        case "getSiblings":
          ...
          if (!isProjectFilePathSafe(msg.filePath, projectRoot)) {
            logger.warn(`[Glide] Rejected siblings path: ${msg.filePath}`);
    ...
    ws.on("close", () => {
      if (ws === activeClient) {
        activeClient = null;
        // Keep undoStack alive across reconnections (HMR page reloads)
        queue.length = 0;
      }
    });
  });
