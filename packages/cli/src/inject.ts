...
export function createProxyServer(
  ...
  const { targetPort, targetHost, proxyPort, wsPort, appFramework, getActiveClient } = options;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (normalizedUrl === "/__glide/overlay.js") {
    ...
    // Serve font files
    if (normalizedUrl === "/__glide/inter-regular.woff2") {
      res.writeHead(200, {
    ...
    if (normalizedUrl === "/__glide/inter-semibold.woff2") {
      res.writeHead(200, {
        "Content-Type": "font/woff2",
        "Cache-Control": "public, max-age=31536000, immutable",
      });
...
<script>window.__REACT_REWRITE_WS_PORT__ = ${wsPort};window.__REACT_REWRITE_APP_FRAMEWORK__ = "${appFramework}";</script>`;
      ...
      // Strip CSP headers that would block the overlay's WebSocket or injected scripts.
