...
import type { SiblingInfo } from "@glide-editor/shared";
...
function classMatchesPrefix(cls: string, prefix: string, breakpoint?: string): boolean {
  if (breakpoint) {
    // When targeting a breakpoint, match "{bp}:{prefix}" or "{bp}:{prefix}-*"
...
function applyUpdate(classes: string[], update: ClassNameUpdate): string[] {
  ...
  // 1. Check relatedPrefixes for shorthand splitting (base classes only, no breakpoint)
  if (!bp) {
  ...
  // 2. Find and replace existing class with same prefix (+ breakpoint if set)
      ...
      for (const quasi of expr.quasis) {
        ...
        for (const update of updates) {
          ...
          if (classes.some((c: string) =>
            allPrefixes.some((p) => classMatchesPrefix(c, p, bp)) ||
            (update.classPattern && new RegExp(bp ? `^${bp}:${update.classPattern.slice(1)}` : update.classPattern).test(c))
          )) {
            hasMatch = true;
            break;
          }
      ...
      for (const update of updates) {
        ...
        for (const arg of args) {
          if (arg.type === "StringLiteral") {
            ...
            if (classes.some((c: string) => allPrefixes.some((p) => classMatchesPrefix(c, p, update.breakpoint)))) {
...
export function mutateTextContent(
  ...
  // Case 3: Text lives in a JSX attribute value (prop), not children.
  // e.g. <StatCard value="1" /> where the DOM renders "1" from the prop.
  const attrs = target.node.openingElement?.attributes;
  if (attrs) {
    for (const attr of attrs) {
      if (attr.type !== "JSXAttribute") continue;
      const val = attr.value;
      if (!val) continue;
      if ((val.type === "StringLiteral" || val.type === "Literal") && String(val.value) === originalText) {
        logger.debug(`[mutateText] Found in attribute "${attr.name?.name}" = "${val.value}" → "${newText}"`);
        val.value = newText;
        return true;
      }
      if (
        val.type === "JSXExpressionContainer" &&
        (val.expression?.type === "StringLiteral" || val.expression?.type === "Literal") &&
        String(val.expression.value) === originalText
      ) {
        logger.debug(`[mutateText] Found in attribute expr "${attr.name?.name}" = "${val.expression.value}" → "${newText}"`);
        val.expression.value = newText;
        return true;
      }
    }
  }
