...
import {
  ...
  mutateVueClass,
  mutateVueText,
  mutateTemplateInlineStyle,
  mutateTemplateReorder,
...
import {
  isAngularComponent,
  getAngularTemplate,
  getAngularTemplateFromSource,
  angularUpdateClass,
  angularUpdateText,
} from "./angular-transform.js";
import { logger } from "./logger.js";
import { detectTailwindVersion } from "./tailwind-resolver.js";
...
function containsText(node: any, text: string): boolean {
  ...
  if (attrs) {
    for (const attr of attrs) {
      ...
      if (!val) continue;
      if ((val.type === "StringLiteral" || val.type === "Literal") && String(val.value) === normalized) return true;
      if (val.type === "JSXExpressionContainer" && (val.expression?.type === "StringLiteral" || val.expression?.type === "Literal") && String(val.expression.value) === normalized) return true;
    }
  }
  return false;
}
...
function resolveNodes(
  ...
  for (const { index, op } of ops) {
    if (op.op === "reorder" || op.op === "duplicate") {
      // Line-based ops — resolution happens during mutation
      resolved.push({
        index,
        op,
        node: null,
    ...
    if (!node) {
      // For updateText, allow fallback to attribute search in applyOp
      if (op.op === "updateText") {
        resolved.push({ index, op, node: null, priority: 0 });
        continue;
      }
      resolved.push({
        index,
...
function applyOp(j: any, root: any, rop: ResolvedOp, source: string): string | undefined {
  ...
  switch (op.op) {
    ...
    case "updateText": {
      // Try direct text mutation if we have a resolved node
      if (node) {
        const found = mutateTextContent(node, op.originalText, op.newText, source, op.cursorOffset);
        if (found) return undefined;
      }

      // Fallback: text may live in a parent component's prop (e.g. <StatCard value="1" />
      // renders {value} in a <p>, so the resolved node is the <p> but the literal is on
      // the <StatCard> usage site). Search for elements with the component name as tag
      // that have the originalText as an attribute value, preferring the one closest to
      // the reported line number.
      const componentTag = op.componentName;
      type AttrMatch = { attr: any; line: number };
      const attrMatches: AttrMatch[] = [];
      root.find(j.JSXElement).forEach((p: any) => {
        const astTag = getJSXTagName(p.node);
        if (componentTag && astTag !== componentTag) return;
        const attrs = p.node.openingElement?.attributes;
        if (!attrs) return;
        const elLine = p.node.openingElement?.loc?.start?.line ?? 0;
        for (const attr of attrs) {
          if (attr.type !== "JSXAttribute") continue;
          const val = attr.value;
          if (!val) continue;
          if ((val.type === "StringLiteral" || val.type === "Literal") && String(val.value) === op.originalText) {
            attrMatches.push({ attr: val, line: elLine });
          } else if (val.type === "JSXExpressionContainer" && (val.expression?.type === "StringLiteral" || val.expression?.type === "Literal") && String(val.expression.value) === op.originalText) {
            attrMatches.push({ attr: val.expression, line: elLine });
          }
        }
      });
    ...
    case "duplicate": {
      // Find the first JSX element at the target line and duplicate it
      const targetLine = op.line;
      const jsxElements = root.find(j.JSXElement);
      let targetNode: any = null;
      jsxElements.forEach((p: any) => {
        if (targetNode) return; // take first match only
        const startLine = p.node.openingElement?.loc?.start.line;
        if (startLine === targetLine) targetNode = p;
      });
      if (!targetNode) return `No JSX element found at line ${targetLine}`;

      // Walk up to find a parent with children array
      let movable = targetNode;
      ...
      if (!movable.parent) return "Cannot duplicate: no parent container found";

      const children = movable.parent.node.children;
      const idx = children.indexOf(movable.node);
      if (idx === -1) return "Cannot duplicate: element not in parent children";

      const cloned = JSON.parse(JSON.stringify(movable.node));

      const wsAfter = idx + 1 < children.length && children[idx + 1]?.type === "JSXText"
        ? JSON.parse(JSON.stringify(children[idx + 1]))
        : null;
      if (wsAfter) {
        children.splice(idx + 1, 0, wsAfter, cloned);
      } else {
...
// ── Vue/Angular template file processing ─────────────────────────────────
...
function processTemplateFile(
  ...
  try {
    ...
    for (const { index, op } of ops) {
      results[index] = {
        op: op.op, file, line: getOpLine(op), success: false,
  ...
  let modifiedFilePath = resolvedPath;
  let modifiedBefore = beforeContent;

  // For Angular with external templateUrl, switch to working on the .html file
  if (isAngularComponent(resolvedPath) && resolvedPath.endsWith(".ts")) {
    const tmpl = getAngularTemplateFromSource(source, resolvedPath);
    if (tmpl && !tmpl.isInline && tmpl.templateFilePath !== resolvedPath) {
      modifiedFilePath = tmpl.templateFilePath;
      modifiedBefore = tmpl.templateContent;
      currentSource = tmpl.templateContent;
    }
  }
  ...
  for (const { index, op } of ops) {
    ...
    try {
      if (op.op === "updateClass") {
        const updates: ClassNameUpdate[] = op.updates.map((u) => ({
          tailwindPrefix: u.tailwindPrefix,
          tailwindToken: u.tailwindToken,
          value: u.value,
          relatedPrefixes: u.relatedPrefixes,
        ...
        const hints = {
          ...
          nthOfType: op.nthOfType,
          id: op.id,
        };

        if (isAngularComponent(resolvedPath)) {
          let tmplContent: string;
          let tmplOffset: number;
          if (modifiedFilePath.endsWith(".html")) {
            tmplContent = currentSource;
            tmplOffset = 0;
          } else {
            const tmpl = getAngularTemplateFromSource(currentSource, resolvedPath);
            if (!tmpl) throw new Error("Could not find Angular template");
            tmplContent = tmpl.templateContent;
            tmplOffset = tmpl.templateOffset;
          }
          const elements = findTemplateElements(tmplContent, tmplOffset, currentSource);
          const target = findVueElement(elements, op.tagName ?? "div", hints);
          if (!target) throw new Error(`No <${op.tagName ?? "div"}> element found in Angular template`);
          currentSource = mutateVueClass(currentSource, target, updates);
        } else {
          currentSource = mutateVueClassInSource(currentSource, op.tagName ?? "div", updates, hints);
        }
        ...
        if (isAngularComponent(resolvedPath)) {
          ...
          const target = findVueElement(elements, op.tagName ?? "div", hints);
          if (!target) throw new Error(`No <${op.tagName ?? "div"}> element found in Angular template`);
          currentSource = mutateVueText(currentSource, target, op.originalText, op.newText);
        } else {
          currentSource = mutateVueTextInSource(
            currentSource, op.tagName ?? "div", op.originalText, op.newText, hints,
          );
        }
        results[index] = { op: op.op, file, line, success: true };
        fileHasChanges = true;
      } else if (op.op === "reorder") {
        let templateSection: { content: string; contentStart: number } | null;
        if (isAngularComponent(resolvedPath)) {
          if (modifiedFilePath.endsWith(".html")) {
            templateSection = { content: currentSource, contentStart: 0 };
          } else {
            const tmpl = getAngularTemplateFromSource(currentSource, resolvedPath);
            if (!tmpl) throw new Error("Could not find Angular template");
            templateSection = { content: tmpl.templateContent, contentStart: tmpl.templateOffset };
          }
        } else {
          templateSection = extractTemplateSection(currentSource);
        }
        if (!templateSection) throw new Error("No template section found");

        const reorderElements = findTemplateElements(
          templateSection.content, templateSection.contentStart, currentSource,
        );
        currentSource = mutateTemplateReorder(currentSource, reorderElements, op.fromLine, op.toLine);
        results[index] = { op: op.op, file, line, success: true };
        fileHasChanges = true;
        ...
        const dupElements = findTemplateElements(
          templateSection.content, templateSection.contentStart, currentSource,
        );
        currentSource = duplicateTemplateElement(currentSource, dupElements, op.line);
        results[index] = { op: op.op, file, line, success: true };
        fileHasChanges = true;
      } else if (op.op === "moveSpacing") {
        const hints = {
          className: op.className,
          nthOfType: op.nthOfType,
          id: op.id,
        ...
        const target = findVueElement(elements, op.tagName ?? "div", hints);
        if (!target) throw new Error(`No <${op.tagName ?? "div"}> element found in template`);

        // Tailwind-aware path: use translate-x/y classes when project has Tailwind
        const twVersion = projectRoot ? detectTailwindVersion(projectRoot) : null;
        if (twVersion) {
          const basePrefix = op.axis === "x" ? "translate-x" : "translate-y";
          const classPattern = `^-?${basePrefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(-|$)`;
          const classPatternRe = new RegExp(classPattern);

          // Read existing translate class to accumulate
          const existingClasses = (target.staticClass || "").split(/\s+/).filter(Boolean);
          let existingPx = 0;
          for (const cls of existingClasses) {
            if (classPatternRe.test(cls)) {
              existingPx = parseTranslateClassPx(cls, basePrefix);
          ...
          if (Math.abs(totalPx) < 0.5) {
            // Remove existing translate class from class attr
            const cleanedClasses = existingClasses.filter(c => !classPatternRe.test(c)).join(" ");
            if (target.staticClass !== cleanedClasses) {
              const classAttr = target.attributes.find(a => a.name === "class");
              if (classAttr) {
                const oldTag = target.openingTag;
                // Handle both double and single-quoted class attributes
                let newTag = oldTag.replace(/\bclass\s*=\s*"[^"]*"/, `class="${cleanedClasses}"`);
                if (newTag === oldTag) {
                  newTag = oldTag.replace(/\bclass\s*=\s*'[^']*'/, `class="${cleanedClasses}"`);
                }
                currentSource = currentSource.slice(0, target.offset) + newTag + currentSource.slice(target.offset + oldTag.length);
              }
            }
          } else {
            const isNeg = totalPx < 0;
            const token = snapPxToSpacingToken(Math.abs(totalPx));
            const updates: ClassNameUpdate[] = [{
              tailwindPrefix: isNeg ? `-${basePrefix}` : basePrefix,
              tailwindToken: token,
              value: "",
              classPattern,
            }];
          ...
          // Fallback: inline transform style for non-Tailwind projects
          const styleAttr = target.attributes.find((a) => a.name === "style");
          const existingStyle = styleAttr?.value ?? "";
          const existingPx = parseTransformTranslate(existingStyle, op.axis);
          const totalPx = existingPx + op.pxDelta;

          const newTransform = setTransformTranslate(
            existingStyle.match(/transform\s*:\s*([^;]+)/)?.[1]?.trim() ?? "",
            op.axis,
            totalPx,
          );

          if (newTransform) {
            currentSource = mutateTemplateInlineStyle(
              currentSource, target, "transform", newTransform,
            );
          } else {
            currentSource = mutateTemplateInlineStyle(
              currentSource, target, "transform", "none",
            );
          }
        }
      ...
      results[index] = {
        op: op.op, file, line, success: false,
  ...
  if (fileHasChanges) {
    try {
      fs.writeFileSync(modifiedFilePath, currentSource, "utf-8");
      undoEntries.push({
...
/** In-memory Vue SFC class mutation (no file I/O). */
function mutateVueClassInSource(
  source: string,
  tagName: string,
  ...
  const elements = findTemplateElements(template.content, template.contentStart, source);
...
/** In-memory Vue SFC text mutation (no file I/O). */
function mutateVueTextInSource(
  source: string,
  tagName: string,
  originalText: string,
  newText: string,
...
export function executeBatch(
  ...
  for (const [file, ops] of byFile) {
    ...
    // ── Vue SFC or Angular template: use template-specific transforms ──
    if (isVueSFC(resolvedPath) || isAngularComponent(resolvedPath)) {
      processTemplateFile(resolvedPath, file, ops, results, undoEntries, projectRoot);
      continue;
    }
