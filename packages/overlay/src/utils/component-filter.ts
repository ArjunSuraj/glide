 ...
 * Ported from react-grab's isValidGrabbableElement with adaptations for Glide.
 */
export function isValidElement(el: Element): boolean {
  ...
  if (el.closest("#glide-root")) return false;
  if (el instanceof HTMLElement && el.hasAttribute("data-glide-interaction")) return false;
  if (el instanceof HTMLElement && el.hasAttribute("data-glide-placeholder")) return false;
