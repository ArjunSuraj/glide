import type { ComponentInfo, ElementIdentity, PropertyGroup } from "@glide-editor/shared";
...
function getRelevantGroups(element: HTMLElement): Set<PropertyGroup> {
  ...
  // Layout: flex/grid containers, elements with children, or children of flex containers
...
function readComputedValues(
  ...
  // Inject parent display for flex-child gating in section renderer
  if (element.parentElement) {
    values.set("__parentDisplay", getComputedStyle(element.parentElement).display);
  }
 ...
 */
function createBreakpointBar(): HTMLDivElement {
  const bar = document.createElement("div");
  bar.setAttribute("style", "display:flex;gap:2px;padding:6px 12px;border-bottom:1px solid rgba(255,255,255,0.08);");
...
function rerenderSections(): void {
  ...
  wrapper.appendChild(createBreakpointBar());
  wrapper.appendChild(container);
  sidebar.replaceContent(wrapper);
}
    ...
    mergeKey,
    {
      ...
      updates: [...state.pendingBatch.values()].map((entry) => {
        ...
        return {
          ...
          breakpoint: activeBreakpoint || undefined,
        };
      }),
    },
...
export function inspect(element: HTMLElement, info: ComponentInfo): void {
  ...
  // Show sidebar
  sidebar.show(info.componentName, info.filePath, info.lineNumber, wrapper);
  if (!info.filePath) {
