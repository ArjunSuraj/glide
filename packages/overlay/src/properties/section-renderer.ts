import type { PropertyDescriptor, PropertyGroup } from "@glide-editor/shared";
...
const FLEX_ONLY_KEYS = new Set(["flexDirection", "justifyContent", "alignItems", "flexWrap", "gap"]);
const GRID_ONLY_KEYS = new Set(["gridTemplateCols", "gridTemplateRows", "columnGap", "rowGap"]);
const FLEX_CHILD_KEYS = new Set(["flexGrow", "flexShrink", "alignSelf", "order"]);
...
function isGridDisplay(currentValues: Map<string, string>): boolean {
  const display = currentValues.get("display") ?? "";
  return display === "grid" || display === "inline-grid";
}

function isParentFlex(currentValues: Map<string, string>): boolean {
  return currentValues.get("__parentDisplay") === "flex" || currentValues.get("__parentDisplay") === "inline-flex";
}
...
export function renderSections(
  ...
  for (const [group, descs] of grouped) {
    // Filter layout descriptors based on current display context
    let filteredDescs = descs;
    if (group === "layout") {
      const isFlex = isFlexDisplay(currentValues);
      const isGrid = isGridDisplay(currentValues);
      const isChildOfFlex = isParentFlex(currentValues);
      filteredDescs = descs.filter(d => {
        if (FLEX_ONLY_KEYS.has(d.key)) return isFlex;
        if (GRID_ONLY_KEYS.has(d.key)) return isGrid;
        if (FLEX_CHILD_KEYS.has(d.key)) return isChildOfFlex;
        return true;
      });
    }
