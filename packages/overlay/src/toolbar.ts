...
export function mountToolbar(onClose: () => void): void {
  ...
  toolbar.innerHTML = `
    ...
    <button class="icon-btn close-btn" title="Minimize Glide">
      ${CLOSE_SVG}
    </button>
  `;

  // Restore button — small floating icon shown when minimized
  const GLIDE_ICON = `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>`;
  restoreBtnEl = document.createElement("button");
  restoreBtnEl.className = "restore-btn";
  restoreBtnEl.title = "Restore Glide";
  restoreBtnEl.innerHTML = GLIDE_ICON;
  ...
  closeBtn!.addEventListener("click", () => minimizeToolbar());
...
export function minimizeToolbar(): void {
  if (minimized) return;
  minimized = true;
  if (toolbarEl) toolbarEl.style.display = "none";
  if (restoreBtnEl) restoreBtnEl.classList.add("visible");
  onMinimize?.();
}

export function restoreToolbar(): void {
  if (!minimized) return;
  minimized = false;
  if (toolbarEl) toolbarEl.style.display = "";
  if (restoreBtnEl) restoreBtnEl.classList.remove("visible");
  onRestore?.();
}
...
export function setOnMinimize(fn: () => void): void { onMinimize = fn; }
export function setOnRestore(fn: () => void): void { onRestore = fn; }

export function destroyToolbar(): void {
  const host = document.getElementById("glide-root");
  if (host) host.remove();
  shadowRoot = null;
  toolbarEl = null;
  restoreBtnEl = null;
