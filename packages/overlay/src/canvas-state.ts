...
/** Runtime extension of textEditRestore — adds DOM element reference for reliable undo. */
type TextEditRestoreRuntime = Extract<CanvasUndoAction, { type: "textEditRestore" }> & {
  element?: HTMLElement;
};
...
let redoStack: CanvasUndoAction[] = [];
...
export function addTextEditAnnotation(
  ...
  undoStack.push(action as CanvasUndoAction);
  notifyStateChange();
}
...
export function canvasUndo(): string | null {
  ...
  switch (action.type) {
    case "moveCreate": {
      // moveCreate cannot be redone (the full MoveEntry is destroyed)
      redoStack.length = 0;
    ...
    case "moveDelta": {
      ...
      if (moveEntry) {
        const currentDelta = { ...moveEntry.delta };
        redoStack.push({ type: "moveDelta", moveId: action.moveId, previousDelta: currentDelta });
    ...
    case "annotationAdd": {
      // annotationAdd cannot be redone (the full annotation is destroyed)
      redoStack.length = 0;
    ...
    case "colorChange": {
      ...
      // colorChange cannot be redone (annotation is destroyed)
      redoStack.length = 0;
      removeAnnotation(action.annotationId);
    ...
    case "propertyChange": {
      ...
      if (propAction.element && document.contains(propAction.element)) {
        const reverseOverrides = propAction.overrides.map(o => ({
          cssProperty: o.cssProperty,
          previousValue: o.newValue,
          newValue: o.previousValue,
        }));
        redoStack.push({
          type: "propertyChange",
          elementIdentity: propAction.elementIdentity,
          overrides: reverseOverrides,
          element: propAction.element,
          pendingMergeKey: propAction.pendingMergeKey,
          pendingPropertyKeys: propAction.pendingPropertyKeys,
        } as PropertyChangeRuntime as CanvasUndoAction);
    ...
    case "textEditRestore": {
      const textAction = action as TextEditRestoreRuntime;
      let el = textAction.element && document.contains(textAction.element)
        ? textAction.element
        : reacquireMovedElement(action.elementIdentity);
      if (el) {
        el.innerHTML = action.originalInnerHTML;
      }
      // textEditRestore cannot be redone (annotation is destroyed)
      redoStack.length = 0;
      removeAnnotation(action.annotationId);
...
export function canvasRedo(): string | null {
  const action = redoStack.pop();
  if (!action) return null;

  // Re-apply the action (push its reverse onto undoStack without clearing redoStack)
  switch (action.type) {
    case "moveDelta": {
      const moveEntry = moves.get(action.moveId);
      if (moveEntry) {
        const currentDelta = { ...moveEntry.delta };
        undoStack.push({ type: "moveDelta", moveId: action.moveId, previousDelta: currentDelta });
        moveEntry.delta = action.previousDelta;
        applyMoveTransform(moveEntry);
      }
      notifyStateChange();
      return "move re-applied";
    }
    case "propertyChange": {
      const propAction = action as PropertyChangeRuntime;
      if (propAction.element && document.contains(propAction.element)) {
      ...
      notifyStateChange();
      return "property re-applied";
    }
    default:
      notifyStateChange();
      return "change re-applied";
  }
}
...
/**
 * Revert all canvas changes — restores original inline styles, text, and moves.
 * Used by the Reset Canvas button to undo visual edits.
 */
...
/**
 * Clear canvas state after a successful commit — removes inline style overrides
 * (instead of reverting them) so the new CSS classes from HMR take effect.
 */
export function clearCanvasAfterCommit(): void {
  clearCanvasState(false);
}

function clearCanvasState(revert: boolean): void {
  for (const entry of moves.values()) {
    if (revert) {
      entry.element.style.cssText = entry.originalCssText;
    }
  ...
  // Revert or clear color overrides
    ...
    if (ann.type === "colorChange") {
      ...
      if (co.targetElement) {
        if (revert) {
          ...
          co.targetElement.style.removeProperty(co.property);
        }
      }
    }
  }
  // Revert or clear property overrides and text edits
      ...
      if (propAction.element && document.contains(propAction.element)) {
        for (const override of propAction.overrides) {
          if (revert) {
            ...
            propAction.element.style.removeProperty(override.cssProperty);
          }
        }
      }
    }
    if (action.type === "textEditRestore") {
      if (revert) {
        const textAction = action as TextEditRestoreRuntime;
        const el = textAction.element && document.contains(textAction.element)
          ? textAction.element
          : reacquireMovedElement(action.elementIdentity);
        if (el) {
          el.innerHTML = action.originalInnerHTML;
