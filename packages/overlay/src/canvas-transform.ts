...
export function initCanvasTransform(): void {
  ...
  wrapper.setAttribute("data-glide-canvas-wrapper", "true");
  ...
  dotBg.setAttribute("data-glide-dot-bg", "true");
  dotBg.style.cssText = `
    position: fixed;
    top: 0;
    left: 0;
  ...
  // Move all non-Glide body children into the wrapper
  ...
  for (const child of bodyChildren) {
    if (child instanceof HTMLElement) {
      ...
      if (
        child.id === "glide-root" ||
        child.hasAttribute("data-glide-interaction") ||
        child.hasAttribute("data-glide-placeholder") ||
        child.hasAttribute("data-glide-annotation") ||
        child.hasAttribute("data-glide-dot-bg") ||
        child.hasAttribute("data-glide-canvas-wrapper")
      ) {
        continue;
      }
    }
  ...
  // Insert bg and wrapper into body (before Glide elements)
