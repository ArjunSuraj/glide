import type { ChangeEntry } from "@glide-editor/shared";
...
/**
 * Revert all active CLI-backed changes, then clear the changelog.
 * Returns true if any revert messages were sent (caller should wait for completion).
 */
export function revertAllCliChanges(): boolean {
  const allUndoIds: string[] = [];

  for (const [id, entry] of entries) {
    if (entry.state === "reverted") continue;
    const rd = entry.revertData;
    if ((rd.type === "cliUndo" || rd.type === "batchApplyUndo") && rd.undoIds?.length) {
      allUndoIds.push(...rd.undoIds);
      pendingRevertLogEntryIds.add(id);
    }
  }
  if (allUndoIds.length > 0) {
    send({ type: "revertChanges", undoIds: allUndoIds });
    return true;
  }
  return false;
}
...
export function initChangelog(shadowRoot: ShadowRoot): void {
  ...
  // Listen for undoHistory (sent by CLI on reconnection after HMR page reload)
  // Rebuild changelog entries so "Reset All" can still revert file changes
  const cleanupUndoHistoryListener = onMessage((msg) => {
    if (msg.type === "undoHistory" && msg.undoIds?.length > 0) {
      // Only rebuild if we have no CLI-backed entries (i.e., we just reloaded)
      const hasCliEntries = Array.from(entries.values()).some(
        e => e.state !== "reverted" &&
          (e.revertData.type === "cliUndo" || e.revertData.type === "batchApplyUndo")
      );
      if (!hasCliEntries) {
        console.debug("[Glide] Rebuilding changelog from CLI undo history:", msg.undoIds);
        addChangeEntry({
          type: "commitBatch",
          componentName: "Previous Session",
          filePath: "",
          summary: `${msg.undoIds.length} change${msg.undoIds.length !== 1 ? "s" : ""} from before reload`,
          state: "active",
          revertData: { type: "batchApplyUndo", undoIds: msg.undoIds },
        });
      }
    }
  });
  ...
  cleanupMessageListener = onMessage((msg) => {
    if (msg.type === "revertComplete") {
      for (const [id, entry] of entries) {
        ...
        if (revertData.type !== "cliUndo" && revertData.type !== "batchApplyUndo") continue;
