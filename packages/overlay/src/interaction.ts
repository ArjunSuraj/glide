...
export function initInteraction(): void {
  ...
  interactionEl.setAttribute("data-glide-interaction", "true");
...
function onWheel(e: WheelEvent): void {
  ...
  if (target?.closest?.("#glide-root")) return;
  handleWheelZoom(e);
}
...
function onSpaceUp(e: KeyboardEvent): void {
  ...
  if (interactionEl) {
    ...
    interactionEl.style.pointerEvents = (tool === "select" || tool === "interact") ? "none" : "auto";
  }
}
...
/**
 * Find the actual page element at a viewport point, looking through all Glide layers.
...
export function getPageElementAtPoint(clientX: number, clientY: number): HTMLElement | null {
  ...
  for (const el of elements) {
    ...
    if (el.hasAttribute("data-glide-interaction")) continue;
    if (el.hasAttribute("data-glide-placeholder")) continue;
